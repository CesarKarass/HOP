...
public class ChatController : ControllerBase
{
    ...
    [HttpGet("mensajes-enviados/{usuarioId}")]
    public async Task<IActionResult> GetMensajesEnviados(int usuarioId)
    {
        try
        {
            SqlParameter[] p = { new SqlParameter("@UsuarioID", usuarioId) };
            DataTable dt = await _db.ExecuteQueryAsync("ObtenerMensajesEnviados", p);

            var lista = dt.AsEnumerable().Select(row => new
            {
                Id = Convert.ToInt32(row["Id"]),
                ReceptorId = Convert.ToInt32(row["ReceptorID"]),
        ...
        {
            return BadRequest(new { error = "Error al obtener mensajes enviados: " + ex.Message });
        }
    }
...
// ============================================
// DTO - DEFINIDO AQUÍ (solo UNA vez)
// ============================================
